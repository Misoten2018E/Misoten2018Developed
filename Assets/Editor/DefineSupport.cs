...
public class SymbolWindow : EditorWindow {
	...
	private const string ITEM_NAME = "Tools/定義の設定";          // コマンド名
	private const string WINDOW_TITLE = "定義の設定";             // ウィンドウのタイトル
	...
	private void Init() {
		...
		mSymbolList = symbolList.Select(c => new SymbolData(c)).ToArray();

		var defineSymbols = PlayerSettings.GetScriptingDefineSymbolsForGroup(EditorUserBuildSettings.selectedBuildTargetGroup).Split(';');
	...
	private void OnGUI() {
		...
		if (GUILayout.Button("Save")) {
			var defineSymbols = mSymbolList.Where(c => c.IsEnable).Select(c => c.Name).ToArray();
